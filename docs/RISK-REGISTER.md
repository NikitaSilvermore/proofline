...
| T20 | Money-only schema doesn't generalise [A] | **CLOSED (schema) 2026-07-16** | `value_confirmed` → `metric_value` + `metric_unit` **shipped** (DECISIONS.md 2026-07-16; migrated on `silvermore` + deployed, verified live); milestones also gained metric_value/unit + `verification_level` + an `evidence` table. `metric_unit` is free text (canonical currency/count/events_per_month/percentage; health/finance units added when needed). Unit-aware *display* + verification UI/integrations = follow-ups. Form→AI template generator = post-pilot (§10) |
